...
class MaxHeap:
    ...
    def remove(self):
        ...
        del self.heap[-1]
        n = n - 2  # update length of heap after removing last element
        self.maxHeapify(0, n)
        return root
...
def sort_movies_incre(names, ratings):
    ...
    sorted_names = []
    ratings_copy = ratings[:]  # make a copy of ratings
    names_copy = names[:]  # make a copy of names
    for i in range(len(ratings)):
        heap.insert(ratings[i]) # Insert the ratings in the heap
    for i in range(len(ratings)):
        max_rating = heap.remove() # Remove the maximum rating from the heap
        max_index = ratings_copy.index(max_rating) # Get the index of the movie with maximum rating
        sorted_names.append(names_copy[max_index]) # Append the name of the movie to the sorted list
        del ratings_copy[max_index] # Remove the rating of the movie from the list
        del names_copy[max_index] # Remove the name of the movie from the list
    return sorted_names
...
# movies_decreasing = sort_movies_batch(names, ratings)
# print("\033[1;33m" +"Movies sorted in decreasing order of ratings:" + "\033[0m")
# for name in movies_decreasing:
#     print(name)

# print()

# movies_increasing = sort_movies_incre(names, ratings)
# print("\033[1;33m" + "Movies sorted in increasing order of ratings:" + "\033[0m")
# for name in movies_increasing:
#     print(name)